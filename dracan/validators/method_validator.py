...
def create_method_validator(rules_config, logger):
    ...
    method_validation_enabled = rules_config.get(
    ...
    def validate_http_method():
        ...
        if request.method not in allowed_methods:
            ...
            )  # Log disallowed method
            return False, (
                jsonify({"error": f"Method {request.method} not allowed"}),
                405,
            )
    ...
    if method_validation_enabled:
        logger.info(
            "HTTP Method validation is enabled. Allowed methods: "
