...
def test_metrics_default_port_enabled():
    ...
    """
    thread = start_dracan_with_env({"ALLOW_METRICS_ENDPOINT": "true"})
...
def test_metrics_custom_port_enabled():
    ...
    thread = start_dracan_with_env(
        {"ALLOW_METRICS_ENDPOINT": "true", "METRICS_PORT": "2000"}
    )
...
def test_metrics_data_gathering():
    ...
    try:
        ...
        assert (
            "http_requests_total" in metrics_data
        )  # Ensure request count metric is present
