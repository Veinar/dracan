...
def test_explicit_disable_in_uri_validation(monkeypatch, rules_config):
    # Explicitly disable URI_VALIDATION_ENABLED in the environment, even though it's enabled in rules_config
    monkeypatch.setenv("URI_VALIDATION_ENABLED", "false")

    # Expect no SystemExit, as explicit disabling via env should be allowed
    try:
        check_env_config_conflicts(rules_config)
    except SystemExit:
        pytest.fail(
            "check_env_config_conflicts() raised SystemExit unexpectedly when URI validation was explicitly disabled via env."
        )
...
def test_explicit_disable_via_env(monkeypatch, rules_config):
    # Set environment variables to explicitly disable settings that are enabled in rules_config
    monkeypatch.setenv("URI_VALIDATION_ENABLED", "false")
    monkeypatch.setenv("JSON_VALIDATION_ENABLED", "false")
    monkeypatch.setenv("SSL_VALIDATION_ENABLED", "false")
    ...
    try:
        ...
        pytest.fail(
            "check_env_config_conflicts() raised SystemExit unexpectedly when explicit disable via env was set."
        )
