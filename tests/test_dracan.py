...
def run_dracan_server():
    ...
    dracan_thread = Thread(
        target=dracan_app.run, kwargs={"host": "0.0.0.0", "port": 5000}
    )
...
def test_post_valid_json(run_dracan_server):
    ...
    response = requests.post(
        "http://127.0.0.1:5000/data", json=valid_data, headers=headers, timeout=5
    )
...
def test_post_invalid_json(run_dracan_server):
    ...
    response = requests.post(
        "http://127.0.0.1:5000/data", json=invalid_data, headers=headers, timeout=5
    )
...
def test_put_valid_json(run_dracan_server):
    ...
    response = requests.put(
        "http://127.0.0.1:5000/update", json=valid_data, headers=headers, timeout=5
    )
    assert response.status_code == 200
...
def test_put_invalid_json(run_dracan_server):
    ...
    response = requests.put(
        "http://127.0.0.1:5000/update", json=invalid_data, headers=headers, timeout=5
    )
    assert response.status_code == 400
    assert "Invalid JSON" in response.json()["error"]
...
def test_delete_request(run_dracan_server):
    ...
    response = requests.delete(
        "http://127.0.0.1:5000/delete", headers=headers, timeout=5
    )
...
def test_payload_size_limiting(run_dracan_server):
    oversized_data = {
        "name": "A" * 2048,
        "age": 30,
    }  # Exceeds max payload size of 1024 bytes
    headers = {
        "Content-Type": "application/json",
        "X-API-KEY": "test_key",
        "Authorization": "Bearer token123.abc.xyz",
    ...
    response = requests.post(
        "http://127.0.0.1:5000/data", json=oversized_data, headers=headers, timeout=5
    )
...
def test_missing_required_header(run_dracan_server):
    ...
    headers = {
        "Content-Type": "application/json"
    }  # Missing "X-API-KEY" and "Authorization"
    response = requests.post(
        "http://127.0.0.1:5000/data", json=valid_data, headers=headers, timeout=5
    )
...
def test_root_post_request(run_dracan_server):
    ...
    response = requests.post(
        "http://127.0.0.1:5000/", json=data, headers=headers, timeout=5
    )
    assert response.status_code == 201
    assert response.json() == {
